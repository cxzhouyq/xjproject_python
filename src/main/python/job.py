                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if response_data.get('retcode') == -9999:
                                fails.append(f"{url}({user_type}_{user_name})请求失败，cookies失效！")
                            break
                        except ValueError:
                            fails.append(f"{url}({user_type}_{user_name})请求失败，非json返回值")
                    elif domain == domains[-1]:
                        fails.append(f"{url}({user_type}_{user_name})请求失败！")
